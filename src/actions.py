...
class ZoomIn(ActionInstance):
    ...
    def on_action():
        try:
            fn = app.activeWindow().focusWidget().parent().parent().on_zoom_in
        except AttributeError:
            fn = lambda: None
        fn()
...
class ZoomOut(ActionInstance):
    ...
    def on_action():
        try:
            fn = app.activeWindow().focusWidget().parent().parent().on_zoom_out
        except AttributeError:
            fn = lambda: None
        fn()
...
class MoveSideBarPositionForward(ActionInstance):
    _instance = None
    default_shortcut = "tab"
    ...
    def on_action():
        try:
            fn = app.activeWindow().on_move_side_bar_forward
        except AttributeError:
            fn = lambda: None
        fn()

    def on_update_language(self):
        self._action.setText(tr("Move Side Bar Position Forward"))


class MoveSideBarPositionBackward(ActionInstance):
    _instance = None
    default_shortcut = "shift+backtab"
    ...
    def on_action():
        try:
            fn = app.activeWindow().on_move_side_bar_backward
        except AttributeError:
            fn = lambda: None
        fn()

    def on_update_language(self):
        self._action.setText(tr("Move Side Bar Position Backward"))
