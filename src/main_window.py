...
class MainWindow(window.Window):
    ...
    def default_width(self):
        return (
            ...
            * self.splitter.count()
            ...
            settings.dict_side_bar_min_width
            if settings.dict_side_bar_text_editor_show
            else 0
        )
    ...
    def insertion_index(self, open_side, text_editor_):
        if open_side == settings.OpenSide.FAR_LEFT:
            return 0
        if open_side == settings.OpenSide.FAR_RIGHT:
            return self.splitter.count()
        index = self.splitter.indexOf(text_editor_)
        if open_side == settings.OpenSide.LEFT_OF_SELECTED:
            if index == -1:
                return 0
            else:
                return index
        if open_side == settings.OpenSide.RIGHT_OF_SELECTED:
            if index == -1:
                return self.splitter.count()
            else:
                return index + 1
    ...
    def on_move_side_bar_forward(self):
        for text_editor_ in self.text_editors:
            text_editor_.text_edit.text_edit.side_bar_forward()

    def on_move_side_bar_backward(self):
        for text_editor_ in self.text_editors:
            text_editor_.text_edit.text_edit.side_bar_backward()
