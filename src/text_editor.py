...
class TextEditor(QWidget):
    def __init__(self, main_window, state_):
        ...
        self.font_combo_box_container = QWidget()
        self.font_combo_box_container.setLayout(QVBoxLayout())
        self.font_combo_box_container.layout().setContentsMargins(0, 0, 0, 0)
        ...
        self.top.layout().addWidget(self.font_combo_box_container, 0)
        ...
        self.text_edit.font_combo_box.setCurrentText(
            font_family or settings.text_editor_font_family
        )
        self.text_edit.font_size = font_size or settings.text_editor_font_size
    ...
    def state(self):
        return (
            ...
            self.locked,
            self.dictionary,
            self.text_edit.font_family,
            self.text_edit.font_size,
    ...
    def resizeEvent(self, event):
        ...
        self.font_combo_box_container.setFixedWidth(
            self.dict_combo_box.width()
        )
    ...
    def on_dict_combo_hide_popup(self, name):
        ...
        if os.path.exists(path):
            ...
            if self.text_edit.text_edit.side_bar is not None:
                self.text_edit.text_edit.side_bar.deleteLater()
            db = dict_database.DictDatabase(path)
            self.text_edit.text_edit.popup = dict_popup.DictPopup(
                self.text_edit.text_edit,
                db,
            )
            self.text_edit.text_edit.side_bar = dict_view.DictView(db)
            self.text_edit.update_splitter()
            db.on_change_fns.add(self.text_edit.text_edit.pop_refresh)
            db.on_change_fns.add(self.text_edit.text_edit.side_bar_refresh)
            ...
            self.text_edit.text_edit.side_bar = None
            self.prev_dict_path = None

    def on_dict_combo_resize_event(self, event):
        self.dict_combo_resize_event(event)
        self.font_combo_box_container.setFixedWidth(event.size().width())
    ...
    def on_update_settings(self):
        self.update_label_text(self.unsaved)
        if settings.text_editor_show_font_combo_box:
            self.font_combo_box_container.layout().addWidget(
                self.text_edit.font_combo_box
            )
        else:
            self.text_edit.font_combo_box.setParent(None)
        content_margin = int(
