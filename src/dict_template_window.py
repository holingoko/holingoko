...
class DictTemplateWindow(form_list_edit_window.FormListEditWindow):
    def __init__(self, db, entry_id=None):
        ...
        self.forms = QWidget()
        self.test = QWidget()
        self.forms.setLayout(QVBoxLayout())
        self.test.setLayout(QVBoxLayout())
        self.forms.layout().addWidget(self.forms_label)
        self.forms.layout().addWidget(self.forms_frame)
        self.forms.layout().setContentsMargins(0, 0, 0, 0)
        ...
        self.test.sizeHint = lambda: QSize(
            self.forms.sizeHint().width(),
            self.forms.sizeHint().height() // 2,
        )
        self.form_splitter = splitter.Splitter()
        self.form_splitter.setOrientation(Qt.Orientation.Vertical)
        self.form_splitter.addWidget(self.forms)
        self.form_splitter.addWidget(self.test)
        self.preview = QWidget()
        self.preview.setLayout(QVBoxLayout())
        self.preview.layout().addWidget(self.preview_label, 0)
        self.preview.layout().addWidget(self.dict_view, 1)
        self.splitter = splitter.Splitter()
        self.splitter.setOrientation(Qt.Orientation.Horizontal)
        self.form_splitter.sizeHint = lambda: QSize(
    ...
    def on_update_settings(self):
        ...
        self.form_splitter.setParent(None)
        self.preview.setParent(None)
        self.form_splitter.setGeometry(0, 0, 1, 1)
        self.preview.setGeometry(0, 0, 1, 1)
        if not settings.dict_side_bar_entry_preview_show:
            self.splitter.addWidget(self.form_splitter)
            preview_right = False
            preview_left = False
        elif (
            settings.dict_side_bar_entry_preview_side
        ...
        self.form_splitter.setContentsMargins(
            content_margin - (handle_width if preview_left else 0),
            content_margin,
            content_margin - (handle_width if preview_right else 0),
            content_margin,
        )
        self.preview.layout().setContentsMargins(
            content_margin - (handle_width if preview_right else 0),
            content_margin,
            content_margin - (handle_width if preview_left else 0),
            content_margin,
        ...
        self.forms.layout().setSpacing(spacing)
        self.test.layout().setSpacing(spacing)
        self.preview.layout().setSpacing(spacing)
